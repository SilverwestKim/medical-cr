...
sub = "https://www.kha.or.kr/impart/notice/list?&keyWord=환자안전&menuIdx=502&rMnuGb=IMP&siteCd=HOME&siteGb=HOME&page="
url = c()
for (k in 1:10){
  url[k] = paste0(sub,k)
}
...
for (p in 1:length(url)){
  ...
  text = c()
  for (g in 1:10){
    subly = "https://www.kha.or.kr/impart/notice/view?&keyWord=환자안전&menuIdx=502&rMnuGb=IMP&siteCd=HOME&siteGb=HOME&page="
    hp = "&brdIdx="
    texturl = c()
...
write.csv(final, "Notice.csv", row.names=FALSE)
